...
public class Formula {
    ...
    public ArrayList<Object> formulaExecute(Expression expression, SelectResult selectResult) throws TMDBException {
        ...
        switch (type) {
            ...
            case "SignedExpression":
                ...
                break;
            case "Function":
                dataList = function((net.sf.jsqlparser.expression.Function) expression, selectResult);
                break;
        }
        return dataList;
    }
    ...
    // 聚合函数处理（AVG, SUM, COUNT, MIN, MAX）
    public ArrayList<Object> function(Function function, SelectResult selectResult) throws TMDBException {
        String funcName = function.getName();
        int tupleCount = selectResult.getTpl().tuplelist.size();
        ArrayList<Object> res = new ArrayList<>();
        ...
        if (function.getParameters() != null && !function.getParameters().getExpressions().isEmpty()) {
            ...
            if (param instanceof Column) {
                paramName = ((Column) param).getColumnName();
            } else {
                paramName = param.toString();
            }
        }

        // 查找参数列在 selectResult 中的下标
        int colIndex = -1;
        if (paramName != null) {
            for (int i = 0; i < selectResult.getAttrname().length; i++) {
                if (paramName.equals(selectResult.getAttrname()[i])
                        || paramName.equals(selectResult.getAlias()[i])) {
                    colIndex = i;
                    break;
                }
            }
        ...
        if (colIndex < 0 && !"count".equalsIgnoreCase(funcName)) {
            throw new TMDBException(ErrorList.COLUMN_NAME_DOES_NOT_EXIST,
                    paramName != null ? paramName : funcName);
        }

        // 收集所有行的值
        ArrayList<Double> values = new ArrayList<>();
        for (Tuple tuple : selectResult.getTpl().tuplelist) {
            if (colIndex >= 0 && tuple.tuple[colIndex] != null) {
                values.add(Double.parseDouble(tuple.tuple[colIndex].toString()));
            }
        }
        ...
        switch (funcName.toLowerCase()) {
            ...
            case "max":
                ...
                break;
            case "count":
                result = colIndex >= 0 ? values.size() : tupleCount;
                break;
            default:
                throw new TMDBException(ErrorList.TYPE_IS_NOT_SUPPORTED, "function " + funcName);
        }
