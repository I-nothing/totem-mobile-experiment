    ...
    public SelectResult plainSelect(SelectBody stmt) throws TMDBException, IOException {
        ...
        if(plainSelect.getGroupBy() != null){
            ...
            if (plainSelect.getHaving() != null) {
                selectResult = new Having().having(plainSelect, selectResult);
            }
        }
...
//            这里是针对selectItem是表达式的判定，a或者a+2或者a+b*c都会被认定为表达式
            if(selectItemList.get(i).getClass().getSimpleName().equals("SelectExpressionItem")){
                ...
                ||(selectItem.getAlias()!=null && selectItem.getAlias().getName().equals(groupByElement))){
                    ...
                    boolean countAll = "count".equalsIgnoreCase(funcName)
                            && (func.isAllColumns() || (func.getParameters() != null
                            && !func.getParameters().getExpressions().isEmpty()
                            && "*".equals(func.getParameters().getExpressions().get(0).toString())));
                    String p = countAll ? null
                            : func.getParameters().getExpressions().get(0).toString();
                    int pIndex=-1;
                    if (p != null) {
                    ...
                    if(pIndex==-1 && !countAll){
                        throw new TMDBException(/*2, p*/);
                    }
                    thisColumn=solveAggregationFunction(resultMap,funcName,pIndex);
                }
                int tempI=-1;
                Column column = map.get(selectItem).isEmpty() ? null : map.get(selectItem).get(0);
                for (int j = 0; j < selectResult.getClassName().length; j++) {
                    if(column == null) {
                        if (selectResult.getAttrname()[j].equals(groupByElement)
                                || selectResult.getAlias()[j].equals(groupByElement)) {
                            tempI = j;
                            break;
                        }
                    }
                ...
                for(int j=0;j<resTupleList.tuplelist.size();j++){
                    ...
                    if (tempI >= 0 && j < selectResult.getTpl().tuplelist.size()) {
                        resTupleList.tuplelist.get(j).tupleIds[i]=selectResult.getTpl().tuplelist.get(j).tupleIds[tempI];
                    }
                }

                result.getType()[i]=tempI >= 0 ? selectResult.getType()[tempI] : "int";
                result.getAttrid()[i]=tempI >= 0 ? selectResult.getAttrid()[tempI] : 0;
                result.getClassName()[i]=tempI >= 0 ? selectResult.getClassName()[tempI]
                        : selectResult.getClassName()[0];
                i++;
            }
        }
    ...
    private ArrayList<Object> solveAggregationFunction(HashMap<Object,ArrayList<Tuple>> resultMap, String funcName, int pIndex) {
        ...
        for (Object k :
            ...
            if (pIndex < 0 && "count".equalsIgnoreCase(funcName)) {
                for (int i = 0; i < tuples.size(); i++) temp.add(1.0);
            } else {
                for (Tuple t : tuples) {
                    if(t.tuple[pIndex]!=null) {
                        temp.add(Double.parseDouble(t.tuple[pIndex].toString()));
                    }
                }
            }
    ...
    private void projectSelectExpression(SelectItem item, SelectResult entireResult,
       ...
       String originalName = fullOriginalName.contains(" ") ? fullOriginalName.substring(0, fullOriginalName.indexOf(" ")) : fullOriginalName;
        ...
        // 3.resTupleList赋值
        // 调用Formula对表达式进行解析，获取该列的值
        ArrayList<Object> dataList = (new Formula()).formulaExecute(selectItem.getExpression(), entireResult);
        // 找到表达式对应属性在原元组对应的下标
        int indexInEntireResult = getExpressionIndex(selectItem.getExpression(), entireResult);
        for (int  i = 0;  i< resTupleList.tuplelist.size(); i++) {
            ...
            resTupleList.tuplelist.get(i).tupleIds[indexInResult] = indexInEntireResult >= 0
                    ? entireResult.getTpl().tuplelist.get(i).tupleIds[indexInEntireResult] : 0;
            projectResult.getAttrid()[indexInResult] = indexInEntireResult >= 0
                    ? entireResult.getTpl().tuplelist.get(i).tupleIds[indexInEntireResult] : 0;
        }
        // 4.剩余属性赋值
        projectResult.getType()[indexInResult] = indexInEntireResult >= 0
                ? entireResult.getType()[indexInEntireResult] : "int";
        projectResult.getAttrid()[indexInResult] = indexInEntireResult >= 0
                ? entireResult.getAttrid()[indexInEntireResult] : 0;
        projectResult.getClassName()[indexInResult] = indexInEntireResult >= 0
                ? entireResult.getClassName()[indexInEntireResult]
                : entireResult.getClassName()[0];
    }

    /**
     * 给定表达式，找到它在原始查询结果中对应的列下标。
     * 对于聚合函数（如 AVG(salary)），返回函数参数的列下标。
     */
    private int getExpressionIndex(Expression expression, SelectResult entireResult) {
        if (expression instanceof Function) {
            Function func = (Function) expression;
            if (func.getParameters() != null && !func.getParameters().getExpressions().isEmpty()) {
                Expression param = func.getParameters().getExpressions().get(0);
                if (param instanceof Column) {
                    String colName = ((Column) param).getColumnName();
                    String tableName = ((Column) param).getTable() != null
                            ? ((Column) param).getTable().getName() : "";
                    return getIndexInEntireResult(entireResult, tableName, colName);
                }
            }
            return 0; // COUNT(*) 等 fallback
        }
        ArrayList<String> TableColumn = new ArrayList<>();
        attributeParser(expression.toString(), TableColumn);
        return getIndexInEntireResult(entireResult, TableColumn.get(0), TableColumn.get(1));
    }
